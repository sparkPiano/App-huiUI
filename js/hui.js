...
var baseUrl = "http://192.168.0.168:8080/shopH5";
...
var hui = (function(selector){
	...
	_hui.ajax = function(sets){
		...
		sets.complete = 'complete' in sets ? sets.complete : function(){
			var status = xhr.status;
			if(status == 401 || status == 403){
				var _href = document.location.href;
				if(_href.indexOf("views")>0){
		...
		if(sets.type == 'GET'){
			if(sets.url.indexOf("?") > 0){
				sets.url = sets.url + "&sid="+localStorage.getItem("sid");
			}else{
				sets.url = sets.url + "?sid="+localStorage.getItem("sid");
			}
		}
		...
		if(sets.type == 'POST'){
			...
			if(sets.ContentType == 'application/x-www-form-urlencoded'){
				...
				pd += "sid="+localStorage.getItem("sid");//线上注掉
